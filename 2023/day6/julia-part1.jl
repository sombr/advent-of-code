...
function count_winning_times(time::Int32, dist::Int32)::Int32
    D = time^2 - 4 * dist
    ...
    if Float32(range_start_int) - range_start == 0
        range_start_int += 1
    end
    if Float32(range_end_int) - range_end == 0
        range_end_int -= 1
    end
...
function main(filename)
    ...
    times = [ parse(Int32, strip(n)) for n in split(strip(split(times, ":")[2]), r"\s+") ]
    dists = [ parse(Int32, strip(n)) for n in split(strip(split(dists, ":")[2]), r"\s+") ]
